...
version=$(om interpolate --config "${__DIR}/../versions.yml" --path /pks_version)
files_directory="${__DIR}/../pcf/files/$ENVIRONMENT_NAME"
...
version=$(om -t "$OM_TARGET" --skip-ssl-validation available-products --format json | \
       jq -r '.[] | select(.name=="pivotal-container-service") | .version')
